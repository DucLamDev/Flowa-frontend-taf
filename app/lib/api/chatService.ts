...
export const chatService = {
  ...
  createSession: async (sessionData: Partial<ChatSession>) => {
    const response = await axiosInstance.post('/chat/sessions', sessionData);
    return response.data;
  },

  updateSession: async (sessionId: string, sessionData: Partial<ChatSession>) => {
    const response = await axiosInstance.put(`/chat/sessions/${sessionId}`, sessionData);
    return response.data;
  },
  ...
  sendMessage: async (sessionId: string, message: string) => {
    const response = await axiosInstance.post(`/chat/sessions/${sessionId}/messages`, { content: message });
    return response.data;
  },

  // Content generation via chat
  generateContentFromChat: async (sessionId: string, prompt: string, options: any = {}) => {
    const response = await axiosInstance.post(`/chat/sessions/${sessionId}/generate`, { prompt, options });
    return response.data;
  },

  // Brand-specific chat
  getBrandChatSessions: async (brandId: string) => {
    const response = await axiosInstance.get(`/chat/brands/${brandId}/sessions`);
    return response.data;
  },
  ...
  getChatAnalytics: async () => {
    const response = await axiosInstance.get('/chat/analytics');
    return response.data;
  },
};
